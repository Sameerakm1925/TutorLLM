...
const adminController = (
  ...
  const getUnverifiedCourses = asyncHandler(async (req: Request, res: Response) => {
    const courses = await dbRepositoryCourse.getUnverifiedCourses();
    res.status(200).json({
      status: 'success',
      message: 'Successfully retrieved unverified courses',
      data: courses
    });
  });

  const verifyCourse = asyncHandler(async (req: Request, res: Response) => {
    const { courseId } = req.params;
    await dbRepositoryCourse.verifyCourse(courseId);
    res.status(200).json({
      status: 'success',
