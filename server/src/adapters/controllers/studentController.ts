...
import { submitOnboardingU, adaptPathU } from '../../app/usecases/student';
...
const studentController = (
  ...
  const submitOnboarding = asyncHandler(async (req: CustomRequest, res: Response) => {
    const studentId: string | undefined = req.user?.Id;
    const onboardingData = req.body;
    const updatedStudent = await submitOnboardingU(studentId, onboardingData);
    await dbRepositoryCache.clearCache(studentId ?? '');
    res.status(200).json({
      status: 'success',
      message: 'Successfully generated personalized leaning path',
      data: updatedStudent
    });
  });

  const adaptPath = asyncHandler(async (req: CustomRequest, res: Response) => {
    const studentId: string | undefined = req.user?.Id;
    const { moduleId, lessonId, score } = req.body;
    const updatedStudent = await adaptPathU(studentId, moduleId, lessonId, score);
    res.status(200).json({
      status: 'success',
      message: 'Path adapted based on performance',
      data: updatedStudent
    });
  });
