...
import { submitQuizU } from '../../app/usecases/quiz/submitQuiz';
...
const courseController = (
  ...
  const addLesson = asyncHandler(async (req: CustomRequest, res: Response) => {
    ...
    const questions = lesson.questions && lesson.questions !== 'undefined' ? JSON.parse(lesson.questions) : [];
...
const editLesson = asyncHandler(async (req: CustomRequest, res: Response) => {
    try {
      ...
      const medias = req.files as Express.Multer.File[];
      console.log('lesson body:', JSON.stringify(lesson));
      const questions = lesson.questions && lesson.questions !== 'undefined' ? JSON.parse(lesson.questions) : [];
      lesson.questions = questions;
      await editLessonsU(
        medias,
  ...
  const getQuizzesByLesson = asyncHandler(
    async (req: Request, res: Response) => {
      const lessonId = req.params.lessonId;
      const quizzes = await getQuizzesLessonU(
        lessonId,
        dbRepositoryQuiz,
        dbRepositoryLesson,
        dbRepositoryStudent
  ...
  const submitQuiz = asyncHandler(
    async (req: CustomRequest, res: Response) => {
      const lessonId = req.params.lessonId;
      const studentId = req.user?.Id || '';
      const { answers, attempt } = req.body;
      const result = await submitQuizU(
        studentId,
        lessonId,
        answers,
        dbRepositoryQuiz,
  ...
  const replyDiscussion = asyncHandler(async (req: CustomRequest, res: Response) => {
    const discussionId: string = req.params.discussionId;
    const { message } = req.body.reply;
    const userId = req.user?.Id ?? '';
    const role = req.user?.role ?? 'student';
    const reply = { userId, role, message };
