...
export const instructorRegister = async (
  ...
  const isMobileAlreadyRegistered = await instructorRepository.getInstructorByMobile(
    instructor.mobile.toString()
  );

  if (isMobileAlreadyRegistered) {
    throw new AppError(
      'Instructor with the same mobile number already exists..!',
      HttpStatusCodes.CONFLICT
    );
  }
